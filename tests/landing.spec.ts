...
test.describe('Landing Page E2E Tests', () => {
  ...
  test('LP-T1-4: Newsletter validation', async ({ page }) => {
    ...
    await emailInput.fill('test@agenttag.dev');
  ...
  test('LP-T1-5: FAQ Accordion', async ({ page }) => {
    ...
    const secondFaq = page.locator('.ad-article').nth(1);
    const secondFaqBtn = secondFaq.locator('.ad-article-head');
  ...
  test('LP-T2-3: FAQ accordion toggling same item twice collapses it', async ({ page }) => {
    await page.goto('/#/support');
    const firstFaq = page.locator('.ad-article').first();
    const firstFaqBtn = firstFaq.locator('.ad-article-head');
