...
function HomeClient() {
  ...
  const [ledger, setLedger] = useState(() => {
    const initialHashes = ["0x476e0c…", "0x986cdb…", "0x3a9b1c…", "0x8f2d4e…", "0x5c7b9a…", "0x1d4e6f…", "0x7a8b9c…"];
    return ledgerPool.slice(0, 7).map((e, i) => ({ ...e, seq: 1304 + i, hash: initialHashes[i] || genHash(), key: i }));
  });
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (endpoint) {
      ...
      .then(() => {
        setNewsletterStatus("Subscribed successfully! We will be in touch with updates.");
      })
      .catch(err => {
        console.error(err);
      ...
      setNewsletterStatus("Joined the waitlist! We will be in touch.");
    }
    ...
    <div style={{display: "flex", alignItems: "center", gap: "34px"}}>
      ...
      <div className="hide-sm" style={{display: "flex", alignItems: "center", gap: "2px"}}>
        ...
        <div className={`nav-item ${dropdownOpen ? 'is-active' : ''}`} onMouseEnter={handleDropdownEnter} onMouseLeave={handleDropdownLeave}>
          <a className="nav-link" href="#how" aria-haspopup="true" aria-expanded={dropdownOpen} role="button" onClick={handleDropdownClick}>
            Platform
...
<h2 className="display" style={{margin: "0 0 16px", fontSize: "clamp(33px, 4.6vw, 54px)", lineHeight: "1.05"}}>How does the audit ledger <span className="accent-it">secure agent actions?</span></h2>
...
<h2 className="display" style={{margin: "0 0 16px", fontSize: "clamp(33px, 4.6vw, 54px)", lineHeight: "1.05"}}>Why choose delegated identity over <span className="accent-it">shared keys?</span></h2>
...
<div className="faq-list">
                ...
                <div className={"faq-item" + (openFaq === 0 ? " is-open" : "")} key={0}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 1 ? " is-open" : "")} key={1}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 2 ? " is-open" : "")} key={2}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 3 ? " is-open" : "")} key={3}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 4 ? " is-open" : "")} key={4}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 5 ? " is-open" : "")} key={5}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 6 ? " is-open" : "")} key={6}>
                  <button
                    className="faq-q"
                    type="button"
                ...
                <div className={"faq-item" + (openFaq === 7 ? " is-open" : "")} key={7}>
                  <button
                    className="faq-q"
                    type="button"
...
<input className="footer-newsletter-input" name="email" placeholder="Enter your email" required type="email" aria-label="Email for newsletter" value={newsletterEmail} onChange={(e) => setNewsletterEmail(e.target.value)} />
<button aria-label="Subscribe" className={`footer-newsletter-btn ${newsletterSent ? 'is-sent' : ''}`} type="submit" style={newsletterSent ? { background: 'var(--ok)' } : undefined}>
  {newsletterSent ? "Sent!" : (
...
<div className="footer-copyright">© Copyright 2026 AgentTag.me</div>
...
<div className="footer-watermark">AgentTag</div>
      ...
      {showConsent && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-background border-t border-border z-50 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-muted-foreground shadow-2xl">
          <p>
            We use cookies to improve your experience and for analytics. By continuing to use this site, you agree to our <a href="/privacy" className="underline hover:text-foreground">Privacy Policy</a>.
          </p>
          <div className="flex gap-2 w-full sm:w-auto">
            <button 
              ...
              onClick={() => {
                try {
                  localStorage.setItem('aeg-cookie-consent', 'declined');
                } catch {}
            ...
            <button 
              ...
              onClick={() => {
                try {
                  localStorage.setItem('aeg-cookie-consent', 'accepted');
                } catch {}
                setShowConsent(false);
                window.dispatchEvent(new Event('storage'));
              }}
